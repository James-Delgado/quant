...
describe("Feature Catalog panel", () => {
  ...
  it("shows an explicit monitoring-pending state when stats are null", async () => {
    ...
    expect(screen.getByText("Registered").parentElement?.textContent).toContain(
      "2",
    );
  });
  ...
  it("surfaces the OOS-evidence badge from the ablation status", async () => {
    ...
    expect(
      await screen.findByText("edge", { selector: ".pill" }),
    ).toBeInTheDocument();
    expect(
      screen.getByText("no edge", { selector: ".pill" }),
    ).toBeInTheDocument();
  });
});
