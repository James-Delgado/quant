...
| 6 | Phase 4A exit-gate report and go/no-go for Track A | A written report documenting whether Phase 4A's exit gate (GBM > ARIMA in ≥ 2 of 3 recent regimes, DM p<0.05 in ≥ 1) is met, with explicit go/no-go for Track A (transformers) | complete (2026-06-13) — verdict: **gate FAILED — Track A DEFERRED.** Primary arm (GBM signed_returns) loses every required regime (Δ −1.088 / −1.683 / −0.847 vs ARIMA); DM p=1.000 in every required regime; no secondary arm clears Bonferroni. Full evidence + 3 candidate next-direction proposals in `docs/PHASE_4A_REPORT.md`. | [phase-4a-milestone-6-exit-gate.plan.md](../plans/phase-4a-milestone-6-exit-gate.plan.md) |
...
---
*Status: CLOSED (2026-06-13) — exit gate evaluated; verdict in [`docs/PHASE_4A_REPORT.md`](../../docs/PHASE_4A_REPORT.md). Track A deferred; next move per PRD risk table is revisiting features/labels/data/model class — not transformers.*
